...
/*
 * Charger safety-timer enable register. The PMI8998 SMB2 charger IC
 * has two independent safety timers (pre-charge and fast-charge).
 * When enabled, if either timer elapses before the charger reaches
 * natural termination, the IC latches into an error state
 * (SFT_EXPIRE bit in BATTERY_CHARGER_STATUS_2) and refuses to source
...
#define PRE_CHARGE_SAFETY_TIMER_EN_BIT			BIT(0)
...
static const struct smb2_register smb2_init_seq[] = {
	...
	/*
	 * Disable both safety timers -- see CHGR_SAFETY_TIMER_ENABLE_CFG
	 * comment above. Without this the IC latches SFT_EXPIRE after
	 * ~many hours of uninterrupted charging and stops sourcing
	 * current until a physical unplug.
	 */
	{ .addr = CHGR_SAFETY_TIMER_ENABLE_CFG,
	  .mask = FAST_CHARGE_SAFETY_TIMER_EN_BIT |
...
static int smb2_init_hw(struct smb2_chip *chip)
{
	...
	/*
	 * Force a 0 -> 1 edge on CHARGING_ENABLE_CMD. If a previous
	 * probe (or the bootloader) already left bit 0 set, the
	 * regmap_update_bits() above is a no-op, but the SMB2 charger
	 * FSM only clears a latched SFT_EXPIRE / safety-fault on a
	 * fresh enable edge. Do the clear-then-set explicitly with
	 * raw regmap_write() calls so the writes always hit the bus.
	 */
	rc = regmap_read(chip->regmap,
			 chip->base + CHARGING_ENABLE_CMD, &reg);
	if (rc < 0)
		return dev_err_probe(chip->dev, rc,
				     "failed reading CHARGING_ENABLE_CMD\n");
	rc = regmap_write(chip->regmap,
			  chip->base + CHARGING_ENABLE_CMD,
			  reg & ~CHARGING_ENABLE_CMD_BIT);
	if (rc < 0)
		return dev_err_probe(chip->dev, rc,
				     "failed clearing CHARGING_ENABLE_CMD\n");
	...
	if (rc < 0)
		return dev_err_probe(chip->dev, rc,
				     "failed re-asserting CHARGING_ENABLE_CMD\n");
...
static void smb2_disable_wake_irq(void *data)
{
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	rc = devm_add_action_or_reset(chip->dev, smb2_disable_wake_irq, chip->dev);
	if (rc)
		return rc;
